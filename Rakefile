...
def run_tests(platform, browser, version, junit_dir)
  sh("platform=\"#{platform}\" browserName=\"#{browser}\" version=\"#{version}\" bundle exec parallel_split_test spec") do | success, _exit_code |
    @success &= success
  end
end
...
task :windows_10_ie_11 do
  run_tests('Windows 10', 'internet_explorer', '11', 'junit_reports/windows_10_ie_11')
end
